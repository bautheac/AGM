...
company_id_path <- here::here("data", "H and peck data_Final - working copy")
company_id_map <- readxl::read_xlsx(
    paste0(company_id_path, ".xlsx"), "Dataset",
    range = readxl::cell_limits(c(2L, 2L), c(NA, 3L)),
    col_names = c("company", "id"), col_types = rep("text", 2L)
  ) %>% dplyr::distinct()
...
availables <- dplyr::filter(data_availables, year == 1913L, !is.na(event)) %>% 
...
For the year 1913 and the first 50 firms **with events for that year**, this table displays the number of firms showing **at least one** record for each instrument categories and fields.
          ...
          tryCatch(
            {
              ...
              tibble::tibble(
                `first back` = bound_indexes[1L], `last forward` = bound_indexes[2L]
                )
            },
            error=function(cond) {
...
* Lower bound: 125 days prior to the first event date (usually "Date of signing the letter": LD).  
* Upper bound: 135 days following the last event date (usually "AGM Date": GM; or "Last day of closed books": LC).  
